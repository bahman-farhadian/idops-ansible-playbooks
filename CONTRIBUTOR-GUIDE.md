...
17. Every setting's trailing comment in a tracked `vars/` file ends with
    ...
    column. `vars/settings.local.yml`, generated by `make settings`, carries
    none of this: each setting's own comment is stripped on the way in, so
    the file stays a plain list of live values and cannot drift out of sync
    with the documentation in the tracked file it came from. Read the
    tracked file to learn what a setting means; read the local file only to
...
project exposes, grouped by source file, but with each setting's own inline
comment stripped: what a key means and what values it takes is documented
once, in the tracked file it came from, not duplicated here where it could
drift out of sync. Read the tracked vars file for that; read this file only
for what is actually set. Every value is live, at its current tracked
default (now a blank placeholder), ready to edit in place — nothing needs to
be uncommented before it takes effect. Filling in real values here is what
