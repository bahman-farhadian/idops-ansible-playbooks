...
13. Every project exposes machine-local override files matching
    `*.local.yml`, generated by `make settings`. The default file is
    `vars/settings.local.yml`. Additional files such as
    ...
    `ansible --limit`. This is the only place a real, per-deployment or
    ...
    environment; editing tracked settings files to do it is not. Do not use
    `LIMIT=` / `--limit` against dynamically added hosts: it skips the
    localhost play that runs `add_host`, so the run can exit 0 with no work
    done.
14. Every implementation `Makefile` must include `make settings`, every
    ...
    form), defaulting to `vars/settings.local.yml` and overridable with
    `LOCAL_SETTINGS_FILE`, and the first play must fail immediately with a
    clear message if that file does not exist — a missing `vars_files` entry
    is silently skipped by Ansible rather than erroring, so this guard is
    not optional.
...
```yaml
  vars_files:
    ...
    - "{{ kvm_local_settings_file | default('vars/settings.local.yml') }}"
  ...
  tasks:
    ...
    - name: Fail when settings.local.yml is missing
      ansible.builtin.fail:
        msg: >-
          vars/settings.local.yml is required and was not found. It is the
          only place real, per-deployment values live; the tracked files
          only carry generic defaults. Generate a starting point with:
          make settings
...
2. Append the selected local file — a templated, required entry defaulting
   to `vars/settings.local.yml`, not a nested-list fallback — to every
   play's `vars_files`. Wire `LOCAL_SETTINGS_FILE` from the Makefile as
   `-e <project>_local_settings_file=...`.
...
- `make settings-clean-force` removes only the disposable tier — any
  ...
  touched.
- `make settings-clean-force-all` additionally removes the selected
  `LOCAL_SETTINGS_FILE` (default `vars/settings.local.yml`). This is not
  reversible: it holds real, hand-built configuration that git does not
  back up. Other `*.local.yml` files are left in place. Back up anything
  worth keeping before running it.
...
- The project will not run without `vars/settings.local.yml`. There is no
  tracked fallback for the real, per-deployment values it holds - credentials,
  real hosts - and the first play fails immediately with a clear message if
  the file is missing.
...
Keep one complete local settings file per job instead:
...
kvm-vm-provisioning has adopted this (`LIMIT` removed, `LOCAL_SETTINGS_FILE`
selects the file). Other projects should follow the same Makefile and
playbook pattern when next touched.
...
| Project | Status |
|---------|--------|
| `ac-vm-provisioning/kvm-vm-provisioning` | adopted, including multiple local files via `LOCAL_SETTINGS_FILE` |
| `ag-os-baseline-and-hardening/debian-os-hardening` | adopted (single default file; `LIMIT` still present) |
| `playbook-template` | adopted, so new projects inherit the default file (copy kvm's `LOCAL_SETTINGS_FILE` wiring for multiple files) |
