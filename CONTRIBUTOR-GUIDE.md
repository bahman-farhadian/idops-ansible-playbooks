...
the intended message. Leave that one variable at its real, working value,
and say why in a comment next to it. `debian_hardening_target_group` in
`ag-os-baseline-and-hardening/debian-os-hardening` is the current example: it
fills the same role kvm-vm-provisioning fills with a literal `hosts:
hypervisor`, so it is playbook wiring, not per-environment data.
