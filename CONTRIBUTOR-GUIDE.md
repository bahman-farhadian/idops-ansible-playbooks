...
12. Keep committed settings generic. Values that are true of one machine only
    (real hostnames, SSH keys, local storage paths) never belong in a tracked
    `vars/` file.
13. Expose a machine-local override file, `vars/settings.local.yml`, loaded last
    so it wins, and generated by `make settings`.
14. Every implementation `Makefile` must include `make settings`.
