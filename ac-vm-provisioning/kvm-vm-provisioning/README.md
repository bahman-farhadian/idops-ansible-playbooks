...
- OVMF firmware files `/usr/share/OVMF/OVMF_CODE_4M.ms.fd` and
  `/usr/share/OVMF/OVMF_VARS_4M.ms.fd` (`ovmf` package). Preflight fails
  without them; runtime also asserts the domain XML is OVMF/pflash.

Debian/Ubuntu install command:

```bash
sudo apt update
sudo apt install -y libvirt-clients qemu-utils cloud-image-utils virtinst ovmf acl util-linux
```
...
3. Set `kvm_image_cache_path` and `kvm_instance_disk_pool_path`. These have
   no default and `make provision-check` (or `make provision`) fails until
   you set them - see "Critical Storage Paths Have No Default" below.
   Snapshot overlays are written in the instance disk pool, not a separate
   directory.
...
`kvm_image_cache_path` and `kvm_instance_disk_pool_path` ship blank on
purpose, unlike most other settings in this project. A generic default such
as `/var/lib/libvirt/images/...` can work by coincidence on a host whose
...
one to set. Snapshot overlays are always written in
`kvm_instance_disk_pool_path` (trailing slashes stripped); there is no
separate overlay setting. `make cleanup-force-disks` removes declared disks
and those overlay files from the same directory. Once set,
`kvm_validate_runtime_pool_access` still checks that the libvirt runtime
user (`kvm_libvirt_runtime_user`) can actually reach those paths, and
`kvm_auto_fix_runtime_pool_access` controls whether the role applies an ACL
fix automatically or only reports the problem - that check covers Linux
ACLs, not a MAC layer like AppArmor/SELinux, so a custom path outside what
your host's security policy already allows may still need a manual policy
change even with auto-fix enabled.
...
- external `baseline` snapshot reconciliation remains mandatory for every declared guest
...
every declared domain. Domains missing the snapshot named
`kvm_snapshot_name` (shipped default: `baseline`) resume the completion
flow, including after an interrupted earlier run. Visible QEMU Guest Agent
checkpoints verify `/var/lib/cloud/instance/boot-finished`, that
`ssh.service` is active, and that TCP/22 is listening inside the guest. When
an extra disk uses `filesystem: xfs`, readiness also checks that `mkfs.xfs`
is installed and that the mount point is mounted. It then creates the
mandatory external disk snapshot and verifies its libvirt metadata before
restarting the guest. This is an active readiness gate, not a fixed sleep.
Maximum active readiness wait is `kvm_qemu_guest_agent_wait_timeout_seconds`
(shipped default: 300) per checkpoint.
...
The shipped catalog uses Debian `generic` artifacts (`image_variant: generic`).

Developer note: Debian 13 `generic` booted reliably only with UEFI firmware
...
base disks and writable overlays both live in `kvm_instance_disk_pool_path`
(per-host `instance_disk_pool_path` when restated). Overlay filenames are
`<instance>-<kvm_snapshot_name>-vda.qcow2` (default name `baseline`).
...
generic cases where SATA seed media is not detected early at boot.
...
It also removes the matching `baseline` (or `kvm_snapshot_name`) external
overlays and generated snapshot XML from `kvm_instance_disk_pool_path`. It
never scans or removes snapshot artifacts for undeclared instance names.

## Snapshot Lifecycle

Each successful full provisioning run ensures every declared instance has a
clean-shutdown external disk snapshot named `baseline` (`kvm_snapshot_name`).
It covers `vda` and every configured extra disk. Cloud-init seed media is
detached and deleted after initialization, so it is neither attached to the
running domain nor part of the snapshot. The active domain is restarted on
the new qcow2 overlays in the instance disk pool. Interrupted runs resume
domains that exist but still lack the mandatory snapshot.
...
virsh -c qemu:///system snapshot-dumpxml <instance> baseline
...
declared instance's base disks, seed image, `baseline` overlays, snapshot XML,
and ownership marker from the instance disk pool before `make provision`
...
per-host too. Snapshot overlays are written in that host's instance disk
pool (trailing slashes stripped). The trusted checksum store is the global
setting `kvm_image_checksum_cache_path`. Each path is checked for being
absolute before the run starts.
...
`name` is the inventory name used by `add_host`.
...
Do not use `LIMIT=` / `--limit`. Real hypervisors are added in the first
play on `localhost`. `--limit kvm-host-2` skips that play, so the run can
exit 0 with no work done.

Keep one complete local file per job instead. Each file lists the hosts and
...
Root-disk partition customization is not applied on Debian `generic` images
in this playbook. Use optional extra disks instead. Extra disks with
`filesystem: xfs` install `xfsprogs` in the guest; readiness fails if
`mkfs.xfs` is missing or the mount point is not mounted.
