...
Every tracked file in `vars/` is a blank placeholder; the playbook refuses to
run without `vars/settings.local.yml`, generated by `make settings`. Targets
are declared there, not in `host.yml`: `host.yml` is a static, empty
placeholder, and real hosts are added to the `hardening_targets` inventory
group dynamically at runtime, the same pattern `kvm-vm-provisioning` uses for
its hypervisor fleet. Never hand-edit `host.yml` with a real host's address,
user or password - that would put real infrastructure data in a tracked file.

Set `debian_hardening_targets` in `vars/settings.local.yml`, one list entry
per host:
